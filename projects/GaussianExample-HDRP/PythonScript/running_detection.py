...
class Detection:
    def __init__(self):
        ...
        self.cap = cv2.VideoCapture(1)   # カメラのID指定
    ...
    def detect_waving_hands(self):
        # self.THRESHOLD_LOW = self.img_h / 10
        # self.THRESHOLD_HIGH = self.img_h / 3

        self.THRESHOLD_LOW = 3000
        self.THRESHOLD_HIGH = 15000
        ...
        # 瞬時値で判定
        # if diff_centroid_y > self.THRESHOLD_LOW:
        #     if diff_centroid_y < self.THRESHOLD_HIGH:
        #         speed = (self.MAX_SPEED - 1) / self.THRESHOLD_HIGH * diff_centroid_y + 1
        #         cv2.putText(self.img, ">>{:.2g}".format(speed), 
        #                     (10, 125), cv2.FONT_HERSHEY_SIMPLEX, 5, (0, 0, 255), 10)
        #     else:
        #         cv2.putText(self.img, ">>{}".format(self.MAX_SPEED), (10, 125), cv2.FONT_HERSHEY_SIMPLEX, 5, (0, 0, 255), 10)
        
        
        # 瞬時値の積分値で判定
        if self.y_integral > self.THRESHOLD_LOW:
            if self.y_integral < self.THRESHOLD_HIGH:
                self.speed = (self.MAX_SPEED - 1) / self.THRESHOLD_HIGH * self.y_integral + 1
    ...
    def detect_landmark(self):
        ...
        if self.results.multi_hand_landmarks:
            ...
            for self.h_id, self.hand_landmarks in enumerate(self.results.multi_hand_landmarks):
                ...
                # Res = self.make_hand_landmarks_json()
                # print(Res)
    ...
    def make_hand_landmarks_json(self):
        res = []
        index = 0
        for lm in (self.hand_landmarks.landmark):
            data_p = {}
    ...
    def plot(self):
        ...
        for i in range(len(t)):
            thresh_l.append(self.THRESHOLD_LOW)
            thresh_h.append(self.THRESHOLD_HIGH)
